...
void Meta_Model_command_functor::operator ( )( const string& command, const parameter_info& parameters )
{
   ...
   else if( command == c_cmd_Meta_Model_get )
   {
      ...
      if( field_name.empty( ) )
         ...
         string_getter< numeric >( cmd_handler.p_Meta_Model->Version( ), cmd_handler.retval );
   ...
   else if( command == c_cmd_Meta_Model_set )
   {
      ...
      if( field_name.empty( ) )
         ...
         func_string_setter< Meta_Model, numeric >(
   ...
   else if( command == c_cmd_Meta_Model_cmd )
   {
      ...
      {
         numeric Version( cmd_handler.p_Meta_Model->Version( ) );
         execute_command( Version, cmd_and_args, cmd_handler.retval );
         cmd_handler.p_Meta_Model->Version( Version );
      }
      else if( field_name == c_field_id_Permission || field_name == c_field_name_Permission )
...
void Meta_Model::impl::set_field_value( int field, const string& value )
{
   switch( field )
   {
      ...
      case 13:
      func_string_setter< Meta_Model::impl, numeric >( *this, &Meta_Model::impl::impl_Version, value );
      break;
...
void Meta_Model::impl::validate( unsigned state, bool is_internal, validation_error_container* p_validation_errors )
{
   ( void )state;
   ( void )is_internal;
   ...
   if( !g_add_packages_enum.count( v_Add_Packages ) )
      p_validation_errors->insert( validation_error_value_type( c_field_name_Add_Packages,
...
bool Meta_Model::impl::is_filtered( const set< string >& filters ) const
{
   ( void )filters;
...
const numeric& Meta_Model::Version( ) const
{
   return p_impl->impl_Version( );
}

void Meta_Model::Version( const numeric& Version )
{
...
void Meta_Model::finalise_fetch( )
{
...
bool Meta_Model::is_filtered( const set< string >& filters ) const
{
...
const char* Meta_Model::class_name( ) const
{
   return static_class_name( );
}

const char* Meta_Model::plural_name( ) const
{
   return static_plural_name( );
}

const char* Meta_Model::module_id( ) const
{
   return static_module_id( );
}

const char* Meta_Model::module_name( ) const
{
...
void Meta_Model::do_generate_sql( generate_sql_type type, vector< string >& sql_stmts ) const
{
...
void Meta_Model::static_get_field_info( field_info_container& all_field_info )
{
   ...
   all_field_info.push_back( field_info( "105102", "Version", "numeric", false ) );
...
string Meta_Model::static_get_sql_columns( )
{
   ...
   sql_columns += 
    ...
    "C_Next_Class_Id VARCHAR(128) NOT NULL,"
    "C_Next_List_Id VARCHAR(128) NOT NULL,"
    "C_Next_Specification_Id VARCHAR(128) NOT NULL,"
    "C_Next_View_Id VARCHAR(128) NOT NULL,"
