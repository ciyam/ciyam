...
need to be restarted and then depending upon which of them was rebuilt one of the following bash scripts needs
to be executed:
...
With the second of these bash script files there may be other files required to be included after "<app dir>".
...
It should be noted (which the bash script warns about) that this destroys all existing applications along with
