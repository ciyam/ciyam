...
#                                                                                                           #
# (which if entered via the UI would result in the identity value 6b24210e8)                                #
#                                                                                                           #
# NOTE: Copy and paste the following into your terminal session:                                            #
#-----------------------------------------------------------------------------------------------------------#
 export CDD=bc.e3733d121

 rm -rf .files/*

 mkdir -p $CDD
 rm -rf $CDD/*
 mkdir -p $CDD/e3733d121

 ./ciyam_server -quiet -sid=de604cee0755a3d81944ea96aed12681 -log_files=$CDD -files_area=$CDD 13031 &
 ./ciyam_server -quiet -sid=9a96178dfe775b4baaeed2488c44a54d -no_peers &

 ./ciyam_client -quiet -connect_retries=10 13031
file_archive -add $CDD/e3733d121 10MB e3733d121
...
> file_stats
[10/100000]96.8 kB/100.0 GB:11 tag(s)

> file_archives
e3733d121 [okay      ] (178.4 kB/10.0 MB) bc.e3733d121/e3733d121
...
# NOTE: Copy and paste the following into your terminal session:                                            #
#-----------------------------------------------------------------------------------------------------------#
 ./ods_fsed -quiet $CDD/ciyam_server
cd repo
PREFIX=e3733d121
...
> file_stats
[10/100000]97.1 kB/100.0 GB:11 tag(s)

> file_archives
e3733d121 [okay      ] (178.4 kB/10.0 MB) .files/e3733d121

> file_info bc.e3733d121.zenith
[core-blob] 2f81b61b72185ec053c8f5bfaa65c78ba2ac49f3cf28583701ab25c3f5870c7c (285 B) blk

#-----------------------------------------------------------------------------------------------------------#
# Now shut down the '-log_files=$CDD -files_area=$CDD' server instance using 'fg' and 'ctrl-c'              #
#-----------------------------------------------------------------------------------------------------------#
...
> file_stats
[27/100000]274.0 kB/100.0 GB:29 tag(s)
...
#-----------------------------------------------------------------------------------------------------------#
 mkdir -p tmp
 BLOCK_TREE_HASH=`./ciyam_script block_tree_hash @blockchain_backup_identity`
 export USE_BACKUP_ARCHIVE=1
 BACKUP_LIST_HASH=`./ciyam_script tree_list $BLOCK_TREE_HASH 0 Backup | tail -n 1`
...
;
; ciyam tree hash
~rm $CDD/e3733d121/4bdb8127843f821bacbba8eeaaa5308614d325d1eeddce2c1821b73d00eb62b7
