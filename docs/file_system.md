...
Immediately below the directory where the application server resides is a "files" directory (which by default
is named ".files") that allows the application server to function as a file storage system accessible via the
standard protocol as well as being accessible via the peer protocol.
...
When starting "ciyam_server" the files area path can be modified using the "-files_area=<path>" option.
...
There are three settings in "ciyam_server.sio" configuration file that are relevant to this file system which
...
The files path is set to ".files" by default (being a hidden directory created below the application server's
directory) and can be changed with the path attribute in the server's configuration file or by using a server
startup option. It can also be changed at run-time by setting the system variable "@files_area_path" (with an
automatic "file_resync" taking place when its value is changed).

It may be useful to increase the maximum number of files permitted (assuming there is available disk storage)
but any change to the maximum file size should only be temporary (such as for testing purposes) as this value
needs to be identical between peer connections.

It is not recommended to make any changes to the ".files" directory directly but if this has occurred then it
is possible to resync the application server with the following command:
...
The "file_archive" command is used to add/remove/repair/resize or destroy an archive. The path intended to be
used as an archive is expected to already exist (i.e. it will not be created) as it could be expected that it
is a "mount point".
...
If the path was not accessible then you would instead see the following:
...
used are sixty four characters each (and typically the OS uses quite a lot more bytes per file as overhead).
