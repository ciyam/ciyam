...
If the echo prefix is doubled (i.e. "##") then the echo will only occur if progress output is being permitted
(it can be switched off for "ciyam_client" with the "-no_progress" option). The echo output is also unescaped
so that "#\\#123" would echo "#123" whether or not progress output is permitted.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
'^' - Use this for a simple pause that waits for a key to be pressed (suffix with a custom prompt).
'&' - Use this for either single key selection of choices or a general user appended command input.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For '^' if the custom prompt begins with "X=" (such as "^10=") then pausing will automatically complete after
X seconds when no key is pressed.

If no explicit seconds are set then a CIYAM_PAUSE_SECONDS value (if present) will be used (although a seconds
"countdown" will only be displayed for explicit second values).

To see the seconds countdown but use the CIYAM_PAUSE_SECONDS value use "^0=" as the custom prompt prefix.

If a pause prompt had been displayed and a key had been pressed (rather than a seconds "countdown" finishing)
this can be detected with the CIYAM_KEY_WAS_PRESSED environment variable (which can be useful for scripts).

For user input the following is a selection choice example (sets/clears the environment variable "CONT"):

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
> &Continue? [CONT:Yes==1|No=!NO] (hit 'y', 'Y' or Enter else hit 'n' or 'N')
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

whilst this is a simpler user appended command input example to set the TEST variable to any arbitrary text):

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
> &Test is (enter text value): :TEST=
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

It should be noted that if the environment variable CIYAM_USE_DEFAULT is present then selection choices which
include a default option (such as "Yes==1" in the above Continue example) will have the default option chosen
automatically (so can be used to help automate certain application protocol scripts).

If not using CIYAM_USE_DEFAULT then CIYAM_DEFAULT_SECONDS can be provided to select the default choice option
after a specific number of seconds (for this case a seconds "countdown" will appear after the choice prompt).

In order to better illustrate usage of pause and progress handling the following bash script is used for each
of the "ciyam_client" executions (noting that the CIYAM application server must be running):
...
Example 1:
(./x &) && ./ciyam_client -quiet -exec="<file_progress.cin xx Finished"

This will clear the terminal and initially output something similar to the following:
[xx]
(Starting)
.
Press any key to quit...

Further "dots" on the third line will be output and (if no key is pressed) will finish with this output:
[xx]
(Starting)
..........
(Finished)
...
In this example the "file_progress" application protocol script is using CIYAM_PAUSE_SECONDS and looping with
each repeat clearing the terminal again and calling "cat xx".

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Example 2:
(./x &) && ./ciyam_client -quiet -no_pause -exec="<file_progress.cin xx Finished"

Initial output will be similar to the following:
[xx].

Further "dots" after "[xx]" will appear until they are erased and the script finishes with this output:
[xx]
(Starting)
..........
(Finished)
...
In this example CIYAM_NO_PAUSE is used to detect the "-no_pause" option in order to then output only progress
"dots" (after "[xx]") until completion when the dots are erased and a (one off) "cat xx" occurs.

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Example 3:
(./x &) && ./ciyam_client -quiet -no_pause -no_progress -exec="<file_progress.cin xx Finished"

There is no output initially and even whilst processing but instead only the following final output:
(Starting)
..........
(Finished)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This example is perhaps more likely what might be used when the output is to be redirected although the final
example illustrates another approach for redirection.

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Example 4:
(./x &) && ./ciyam_client -quiet -monitor -no_pause\
 -exec="<file_progress.cin xx Finished" >xxx && echo "" && cat xxx
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Output will appear to be identical to example 2 but the actual content of "xxx" is identical to example 3 (so
the "-monitor" option outputs "[xx]" and the progress "dots" to the console). Thus the use of "-monitor" will
enable progress to be output to the console whilst non-progress output is being redirected to a file.
...
For comparison tests and to create a "ladder" of conditions "@if" and "@elif" can also be used as follows:
...
For numerical comparison tests each operator needs to be prefixed with a "#" character as per the following:
...
Basic logical operators are also supported as follows (where non-numerics are only false if string is empty):
...
@if "$ARG1" #& "$ARG2"
#(both ARG1 and ARG2 non-zero)
@elif "$ARG1" #| "$ARG2"
#(either ARG1 or ARG2 non-zero)
@endif
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Another simplistic looping approach that is suitable for small "list files" is as follows:

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
@label NEXT
MNEMONICS=@file:~grep "^un" mnemonics.txt
;
@:LOOP
;
@ifeq "$MNEMONICS" ""
...
In the example above all mnemonics beginning with "un" are first read into the MNEMONICS environment variable
and in the loop the first line of MNEMONICS is being output (i.e. #%MNEMONICS:1%) after which this same first
line is removed from the variable (i.e. MNEMONICS=%MNEMONICS:-1%).

For larger files the special "@deque" session variable is recommended with the following example illustrating
how it could be used to output all mnemonic words that contain "and". Usage of both "commence" and "complete"
is optional for this example but is necessary if needing to nest "@deque" usage (such as where a script calls
another that is also using the "@deque" session variable).

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
@label NEXT
.session_variable @deque commence
.session_variable @deque "@mnemonics.txt"
;
@:LOOP
...
@:NEXT
;
TEST_LINE=@repstr:and/:$NEXT_LINE
;
@ifneq "$NEXT_LINE" "$TEST_LINE"
#$NEXT_LINE
@endif
;
