...
void Meta_Global_Peerchain_Entry_command_functor::operator ( )( const string& command, const parameter_info& parameters )
{
   ...
   else if( command == c_cmd_Meta_Global_Peerchain_Entry_get )
   {
      ...
      {
         handled = true;
         string_getter< string >( cmd_handler.p_Meta_Global_Peerchain_Entry->Host_Domain( ), cmd_handler.retval );
      }
      ...
      {
         handled = true;
         string_getter< int >( cmd_handler.p_Meta_Global_Peerchain_Entry->Port_Number( ), cmd_handler.retval );
      }
   ...
   else if( command == c_cmd_Meta_Global_Peerchain_Entry_set )
   {
      ...
      if( !handled && field_name == c_field_id_Host_Domain || field_name == c_field_name_Host_Domain )
      {
         handled = true;
         func_string_setter< Meta_Global_Peerchain_Entry, string >(
          *cmd_handler.p_Meta_Global_Peerchain_Entry, &Meta_Global_Peerchain_Entry::Host_Domain, field_value );
      }
      ...
      if( !handled && field_name == c_field_id_Port_Number || field_name == c_field_name_Port_Number )
      {
         handled = true;
         func_string_setter< Meta_Global_Peerchain_Entry, int >(
          *cmd_handler.p_Meta_Global_Peerchain_Entry, &Meta_Global_Peerchain_Entry::Port_Number, field_value );
      }
 ...
 uint64_t state, bool is_internal, validation_error_container* p_validation_errors )
{
   ...
   if( !is_null( v_Host_Domain )
    && ( v_Host_Domain != g_default_Host_Domain
    || !value_will_be_provided( c_field_name_Host_Domain ) )
    && !g_Host_Domain_domain.is_valid( v_Host_Domain, error_message = "" ) )
      p_validation_errors->insert( construct_validation_error( vf.num, c_field_name_Host_Domain,
       get_module_string( c_field_display_name_Host_Domain ) + " " + error_message ) );
...
void Meta_Global_Peerchain_Entry::Host_Domain( const string& Host_Domain )
{
   p_impl->impl_Host_Domain( Host_Domain );
}
...
{
   return p_impl->impl_Port_Number( );
}

void Meta_Global_Peerchain_Entry::Port_Number( int Port_Number )
{
   p_impl->impl_Port_Number( Port_Number );
}
 ...
 const string& id, bool* p_sql_numeric, string* p_type_name ) const
{
   ...
   {
      p_name = c_field_name_Port_Number;
...
string Meta_Global_Peerchain_Entry::get_field_uom_symbol( const string& id_or_name ) const
{
   ...
   else if( id_or_name == c_field_id_Host_Domain || id_or_name == c_field_name_Host_Domain )
   {
      ...
      get_module_string( c_field_display_name_Host_Domain, &next );
   }
   ...
   {
      name = string( c_field_display_name_Port_Number );
      get_module_string( c_field_display_name_Port_Number, &next );
   }
