...
const char* const c_field_id_Entry_Type = "145108";
const char* const c_field_id_Host_Name = "145103";
const char* const c_field_id_Host_Port = "145104";
const char* const c_field_id_Identity = "145101";
const char* const c_field_id_Local_Port = "145105";
const char* const c_field_id_Num_Helpers = "145109";
const char* const c_field_id_Port_Numbers = "145111";
const char* const c_field_id_Status = "145107";
...
const char* const c_procedure_id_Activate = "145430";
const char* const c_procedure_id_Connect = "145410";
const char* const c_procedure_id_Deactivate = "145440";
...
const uint64_t c_modifier_Is_Inactive = UINT64_C( 0x800 );
...
const int c_enum_peerchain_status_Activated( 1 );
...
string get_enum_string_peerchain_status( int val )
{
   ...
   if( to_string( val ) == "" )
      ...
      string_name = "enum_peerchain_status_Activated";
...
void Meta_Global_Peerchain_Entry_command_functor::operator ( )( const string& command, const parameter_info& parameters )
{
   ...
   else if( command == c_cmd_Meta_Global_Peerchain_Entry_get )
   {
      ...
      if( !handled && field_name == c_field_id_Identity || field_name == c_field_name_Identity )
      {
         handled = true;
   ...
   else if( command == c_cmd_Meta_Global_Peerchain_Entry_Activate )
   {
      cmd_handler.p_Meta_Global_Peerchain_Entry->Activate( );
   ...
   else if( command == c_cmd_Meta_Global_Peerchain_Entry_Deactivate )
   {
      cmd_handler.p_Meta_Global_Peerchain_Entry->Deactivate( );
...
void Meta_Global_Peerchain_Entry::impl::impl_Activate( )
{
...
void Meta_Global_Peerchain_Entry::impl::impl_Deactivate( )
{
   uint64_t state = p_obj->get_state( );
   ( void )state;

   // [<start Deactivate_impl>]
//nyi
   set_system_variable( '@' + to_string( get_obj( ).Local_Port( ) ), '~' + get_obj( ).Identity( ) );
   msleep( c_peer_sleep_time );
   // [<finish Deactivate_impl>]
}
...
string Meta_Global_Peerchain_Entry::impl::get_state_names( ) const
{
   ...
   if( state & c_modifier_Is_Inactive )
      state_names += "|" + string( "Is_Inactive" );
 ...
 uint64_t state, bool is_internal, validation_error_container* p_validation_errors )
{
   ...
   if( !is_null( v_Identity )
    && ( v_Identity != g_default_Identity
    || !value_will_be_provided( c_field_name_Identity ) )
 ...
 set< string >& fields_set, validation_error_container* p_validation_errors )
{
   ...
   if( !is_null( v_Identity )
    && ( fields_set.count( c_field_id_Identity ) || fields_set.count( c_field_name_Identity ) )
    && !g_Identity_domain.is_valid( v_Identity, error_message = "" ) )
      p_validation_errors->insert( construct_validation_error( vf.num, c_field_name_Identity,
       get_module_string( c_field_display_name_Identity ) + " " + error_message ) );
...
//nyi
   ...
   if( get_obj( ).Host_Name( ) == c_local_host )
   {
      ...
      {
         get_obj( ).Status( c_enum_peerchain_status_Activated );
         get_obj( ).Actions( '<' + string( c_procedure_id_Deactivate ) );
      }
      else
         get_obj( ).Actions( '<' + string( c_procedure_id_Activate ) );
   ...
   {
      if( !is_null( get_obj( ).Identity( ) ) )
         get_obj( ).Entry_Type( c_enum_peerchain_entry_type_Hosted );
      ...
      else
      {
         if( has_registered_listener( get_obj( ).Local_Port( ), get_obj( ).Identity( ) ) )
         {
            ...
            get_obj( ).Actions( '<' + string( c_procedure_id_Connect ) + ",<" + string( c_procedure_id_Deactivate ) );
         }
         else
            get_obj( ).Actions( '<' + string( c_procedure_id_Activate ) );
      }
...
void Meta_Global_Peerchain_Entry::impl::to_store( bool is_create, bool is_internal )
{
   ...
   // [<start to_store>]
//nyi
   if( get_obj( ).Entry_Type( ) == c_enum_peerchain_entry_type_Local_Only )
...
const string& Meta_Global_Peerchain_Entry::Identity( ) const
{
   return p_impl->impl_Identity( );
}
...
void Meta_Global_Peerchain_Entry::Activate( )
{
   p_impl->impl_Activate( );
}
...
void Meta_Global_Peerchain_Entry::Deactivate( )
{
   p_impl->impl_Deactivate( );
}
 ...
 const string& name, bool* p_sql_numeric, string* p_type_name ) const
{
   ...
   else if( name == c_field_name_Identity )
   {
      p_id = c_field_id_Identity;
 ...
 const string& id, bool* p_sql_numeric, string* p_type_name ) const
{
   ...
   else if( id == c_field_id_Identity )
   {
      p_name = c_field_name_Identity;
...
string Meta_Global_Peerchain_Entry::get_execute_procedure_info( const string& procedure_id ) const
{
   ...
   else if( procedure_id == "145430" ) // i.e. Activate
      retval = "";
   else if( procedure_id == "145410" ) // i.e. Connect
      retval = "";
   else if( procedure_id == "145440" ) // i.e. Deactivate
      retval = "";
 ...
 set< string >& names, bool use_transients, set< string >& dependents ) const
{
   ...
   // [(start modifier_field_value)] 600098a
   dependents.insert( "Status" ); // (for Is_Inactive modifier)
...
{
   all_field_info.push_back( field_info( "145110", "Actions", "string", false, "", "" ) );
   all_field_info.push_back( field_info( "145106", "Auto_Start", "bool", false, "", "" ) );
   all_field_info.push_back( field_info( "145102", "Description", "string", false, "", "" ) );
   all_field_info.push_back( field_info( "145108", "Entry_Type", "int", false, "", "" ) );
   all_field_info.push_back( field_info( "145103", "Host_Name", "string", false, "", "" ) );
   all_field_info.push_back( field_info( "145104", "Host_Port", "int", false, "", "" ) );
   all_field_info.push_back( field_info( "145101", "Identity", "string", false, "", "" ) );
   all_field_info.push_back( field_info( "145105", "Local_Port", "int", false, "", "" ) );
   all_field_info.push_back( field_info( "145109", "Num_Helpers", "int", false, "", "" ) );
   all_field_info.push_back( field_info( "145111", "Port_Numbers", "string", false, "", "" ) );
   all_field_info.push_back( field_info( "145107", "Status", "int", false, "", "" ) );
}
...
procedure_info_container& Meta_Global_Peerchain_Entry::static_get_procedure_info( )
{
   ...
   {
      initialised = true;
      procedures.insert( make_pair( "145430", procedure_info( "Activate" ) ) );
      procedures.insert( make_pair( "145410", procedure_info( "Connect" ) ) );
      procedures.insert( make_pair( "145440", procedure_info( "Deactivate" ) ) );
