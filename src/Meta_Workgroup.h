...
#ifndef META_WORKGROUP_H
#  define META_WORKGROUP_H
...
class Meta_Workgroup : public class_base
{
   ...
   friend class Meta_User;
   friend class Meta_Package;

   public:
   typedef Meta_Workgroup class_type;
   ...
   enum field_id
   {
      ...
      e_field_id_Id = 1,
      e_field_id_Name = 2,
      e_field_id_Next_Enum_Id = 3,
      e_field_id_Next_Model_Id = 4,
      e_field_id_Next_Permission_Id = 5,
      e_field_id_Next_Type_Id = 6,
      e_field_id_Standard_Package = 7
   };
   ...
   const std::string& Next_Enum_Id( ) const;
   void Next_Enum_Id( const std::string& Next_Enum_Id );

   const std::string& Next_Model_Id( ) const;
   void Next_Model_Id( const std::string& Next_Model_Id );

   const std::string& Next_Permission_Id( ) const;
   void Next_Permission_Id( const std::string& Next_Permission_Id );

   const std::string& Next_Type_Id( ) const;
   void Next_Type_Id( const std::string& Next_Type_Id );

   Meta_Package& Standard_Package( );
   const Meta_Package& Standard_Package( ) const;
   void Standard_Package( const std::string& key );

   Meta_Application& child_Application( );
   const Meta_Application& child_Application( ) const;

   Meta_Enum& child_Enum( );
   const Meta_Enum& child_Enum( ) const;

   Meta_Model& child_Model( );
   const Meta_Model& child_Model( ) const;

   Meta_Permission& child_Permission( );
   const Meta_Permission& child_Permission( ) const;

   Meta_Type& child_Type( );
   const Meta_Type& child_Type( ) const;
   ...
   std::string get_func_variable( const std::string& name ) const;
   ...
   static const char* static_class_id( ) { return "101100"; }
   static const char* static_class_name( ) { return "Workgroup"; }
   static const char* static_plural_name( ) { return "Workgroups"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_Workgroup* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_Workgroup( const Meta_Workgroup& );
   Meta_Workgroup& operator =( const Meta_Workgroup& );
   ...
   virtual void setup_graph_parent( Meta_Enum& o, const std::string& foreign_key_field );
   virtual void setup_graph_parent( Meta_Model& o, const std::string& foreign_key_field );
   virtual void setup_graph_parent( Meta_Permission& o, const std::string& foreign_key_field );
   virtual void setup_graph_parent( Meta_Type& o, const std::string& foreign_key_field );
   virtual void setup_graph_parent( Meta_User& o, const std::string& foreign_key_field );
...
inline std::string to_string( const Meta_Workgroup& c ) { return c.get_key( ); }

inline bool is_null( const Meta_Workgroup& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_Workgroup& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_Workgroup& c, const char* p ) { return !( c.get_key( ) == p ); }
