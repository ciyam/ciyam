...
#ifndef META_PACKAGE_H
#  define META_PACKAGE_H
...
class Meta_Package : public class_base
{
   ...
   public:
   typedef Meta_Package class_type;
   ...
   const std::string& Install_Details( ) const;
   void Install_Details( const std::string& Install_Details );
   ...
   Meta_Workgroup& child_Workgroup_Standard( );
   const Meta_Workgroup& child_Workgroup_Standard( ) const;

   Meta_Package_Option& child_Package_Option( );
   const Meta_Package_Option& child_Package_Option( ) const;

   virtual void Cancel_Remove( );

   virtual void Check_Install( );

   virtual void Complete_Remove( );
   ...
   static const char* static_class_id( ) { return "136100"; }
   static const char* static_class_name( ) { return "Package"; }
   static const char* static_plural_name( ) { return "Packages"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_Package* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_Package( const Meta_Package& );
   Meta_Package& operator =( const Meta_Package& );
   ...
   virtual void setup_graph_parent( Meta_Workgroup& o, const std::string& foreign_key_field );
...
inline std::string to_string( const Meta_Package& c ) { return c.get_key( ); }

inline bool is_null( const Meta_Package& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_Package& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_Package& c, const char* p ) { return !( c.get_key( ) == p ); }
