...
void Meta_Relationship::impl::for_store( bool is_create, bool is_internal )
{
   ...
   if( is_null( get_obj( ).Source_Relationship( ) ) )
   {
      ...
      if( !is_destroy_locked_by_own_session( get_obj( ).Child_Class( ) )
       && !is_destroy_locked_by_own_session( get_obj( ).Parent_Class( ) )
       && !is_destroy_locked_by_own_session( rel_field, field_key.c_str( ) ) )
      {
         if( is_create )
...
string Meta_Relationship::get_func_variable( const std::string& name ) const
{
