 ...
 size_t previous_height, string* p_prior_data_tree_hash = 0 )
{
   ...
   if( previous_height )
   {
      ...
      data_tag += to_string( previous_height );

      data_tag += c_dat_suffix;

      string prior_data_hash( tag_file_hash( data_tag ) );

      string prior_data_content(
       construct_blob_for_block_content( extract_file( prior_data_hash, "" ) ) );

      verify_core_file( prior_data_content, false );
...
void process_public_key_file( const string& blockchain,
 const string& hash, size_t height, public_key_scale key_scale, bool has_tertiary = false )
{
   guard g( g_mutex );
   ...
   {
      if( key_scale == e_public_key_scale_secondary )
         has_all_pub_keys = true;
   }
   ...
   else
   {
      ...
      if( !primary_pubkey_hash.empty( ) )
      {
         ...
         if( !has_file( primary_pubkey_hash ) )
         {
            ...
            process_public_key_file( blockchain,
             primary_pubkey_hash, height, e_public_key_scale_primary, has_tertiary );
      }
      ...
      if( !secondary_pubkey_hash.empty( ) )
      {
         if( !has_file( secondary_pubkey_hash ) )
            ...
            process_public_key_file( blockchain,
             secondary_pubkey_hash, height, e_public_key_scale_secondary, has_tertiary );
      }
      ...
      if( !tertiary_pubkey_hash.empty( ) )
      {
         if( !has_file( tertiary_pubkey_hash ) )
            ...
            process_public_key_file( blockchain,
             tertiary_pubkey_hash, height, e_public_key_scale_tertiary, has_tertiary );
      }
...
class socket_command_handler : public command_handler
{
   ...
   bool get_is_legacy_chain( ) const { return is_legacy_chain; }
   void set_is_legacy_chain( bool val ) { is_legacy_chain = val; }
...
void socket_command_handler::get_file( const string& hash_info, string* p_file_data )
{
   ...
   if( is_list )
   {
      ...
      process_list_items( identity, hash, false, 0, 0, &list_items_to_ignore );
   }
   ...
   else if( next_hash_to_put.empty( ) )
   {
      ...
      if( !next_hash.empty( ) )
      {
         ...
         if( next_hash == data_file_hash )
         {
            ...
            // NOTE: If the new tree root hash is the same as at the previous block
            // height then no need to get any tree files.
            if( last_data_tree_is_identical( blockchain, blockchain_height ) )
               get_tree_files = false;

            process_data_file( blockchain, data_file_hash, blockchain_height_pending, 0, get_tree_files );

            if( get_tree_files && !num_tree_items.empty( ) )
...
void peer_session_command_functor::operator ( )( const string& command, const parameter_info& parameters )
{
   ...
   try
   {
      ...
      else if( command == c_cmd_peer_session_get )
      {
         ...
         if( !identity.empty( ) )
         {
            ...
            {
               bool clear_ignore = false;

               if( socket_handler.get_is_legacy_chain( )
                && ( get_hash_tags( hash ).find( c_dat_suffix ) != string::npos ) )
                  clear_ignore = true;

               if( !socket_handler.get_is_legacy_chain( ) )
               {
                  if( clear_ignore_count )
                  {
                     if( --clear_ignore_count == 0 )
                        clear_ignore = true;
                  }
                  else if( get_hash_tags( hash ).find( c_pub_suffix ) != string::npos )
                     clear_ignore_count++;
               }
   ...
   if( !had_zenith_hash && !blockchain_height
    && !get_session_variable( blockchain_zenith_height_name ).empty( ) )
   {
