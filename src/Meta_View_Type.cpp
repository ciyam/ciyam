 ...
 uint64_t state, bool is_internal, validation_error_container* p_validation_errors )
{
   ...
   if( is_null( v_View_Name ) && !value_will_be_provided( c_field_name_View_Name ) )
      p_validation_errors->insert( construct_validation_error( vf.num, c_field_name_View_Name,
       get_string_message( GS( c_str_field_must_not_be_empty ), make_pair(
       c_str_field_must_not_be_empty_field, get_module_string( c_field_display_name_View_Name ) ) ) ) );
...
void Meta_View_Type::impl::completed_fetch( bool skip_set_original )
{
...
void Meta_View_Type::completed_fetch( bool skip_set_original )
{
...
string Meta_View_Type::get_func_variable( const std::string& name ) const
{
