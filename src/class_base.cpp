...
void class_base::perform_after_fetch( bool is_minimal, bool is_for_prepare )
{
   ...
   if( force_fetch || ( !is_minimal && !is_being_cascaded ) )
   {
      ...
      completed_fetch( is_for_prepare );
   }
}
...
string local_backup_blockchain_status( )
{
   ...
   size_t backup_height = from_string< size_t >(
    get_system_variable( e_special_var_blockchain_backup_height ) );
   ...
   if( sync_status.find( c_ellipsis ) == string::npos )
   {
      ...
      for( size_t i = 1; i <= c_max_extras; i++ )
      {
         ...
         if( extra_status.find( c_ellipsis ) != string::npos )
         {
            ...
            if( has_tag( c_bc_prefix + extra_identity + '.' + to_string( backup_height + 1 ) + c_blk_suffix ) )
...
void allow_peerchain( const string& identity, int peer_type, bool no_delay )
{
   ...
   set_system_variable( e_special_var_queue_peers, '@' + identities );
...
void connect_peerchain( const string& connect_info, bool no_delay )
{
   ...
   if( set_system_variable( identity, c_true_value, g_null_string ) )
   {
      ...
      if( set_system_variable( e_special_var_queue_peers, connect_info, check_not_has_either ) )
      {
         string auto_identity_name( get_special_var_name( e_special_var_auto ) + '_' + identity );

         // NOTE: If has "@auto_<identity>" then set it to "1" (in order to reset reconnect attempts).
         if( has_system_variable( auto_identity_name ) )
            set_system_variable( auto_identity_name, c_true_value );
