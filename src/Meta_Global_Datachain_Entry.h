...
#ifndef META_GLOBAL_DATACHAIN_ENTRY_H
#  define META_GLOBAL_DATACHAIN_ENTRY_H
...
#     define META_GLOBAL_DATACHAIN_ENTRY_DECL_SPEC DYNAMIC_EXPORT
#  else
#     define META_GLOBAL_DATACHAIN_ENTRY_DECL_SPEC DYNAMIC_IMPORT
#  endif

class META_GLOBAL_DATACHAIN_ENTRY_DECL_SPEC Meta_Global_Datachain_Entry : public class_base
{
   public:
   typedef Meta_Global_Datachain_Entry class_type;

   enum field_id
   {
      e_field_id_none = 0,
      e_field_id_Chain_Id = 1,
      e_field_id_Data_Type = 2,
      e_field_id_Description = 3
   };

   Meta_Global_Datachain_Entry( );
   ~Meta_Global_Datachain_Entry( );
   ...
   static const char* static_class_id( ) { return "142100"; }
   static const char* static_class_name( ) { return "Global_Datachain_Entry"; }
   static const char* static_plural_name( ) { return "Global_Datachain_Entries"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_Global_Datachain_Entry* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_Global_Datachain_Entry( const Meta_Global_Datachain_Entry& );
   Meta_Global_Datachain_Entry& operator =( const Meta_Global_Datachain_Entry& );
...
inline std::string to_string( const Meta_Global_Datachain_Entry& c ) { return c.get_key( ); }

inline bool is_null( const Meta_Global_Datachain_Entry& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_Global_Datachain_Entry& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_Global_Datachain_Entry& c, const char* p ) { return !( c.get_key( ) == p ); }
