...
#ifndef META_LIST_TYPE_H
#  define META_LIST_TYPE_H
...
#     define META_LIST_TYPE_DECL_SPEC DYNAMIC_EXPORT
#  else
#     define META_LIST_TYPE_DECL_SPEC DYNAMIC_IMPORT
#  endif

class Meta_List;

class META_LIST_TYPE_DECL_SPEC Meta_List_Type : public class_base
{
   friend class Meta_List;

   public:
   typedef Meta_List_Type class_type;
   ...
   Meta_List& child_List_Type( );
   const Meta_List& child_List_Type( ) const;
   ...
   static const char* static_class_id( ) { return "121100"; }
   static const char* static_class_name( ) { return "List_Type"; }
   static const char* static_plural_name( ) { return "List_Types"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_List_Type* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_List_Type( const Meta_List_Type& );
   Meta_List_Type& operator =( const Meta_List_Type& );
...
inline std::string to_string( const Meta_List_Type& c ) { return c.get_key( ); }

inline bool is_null( const Meta_List_Type& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_List_Type& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_List_Type& c, const char* p ) { return !( c.get_key( ) == p ); }
