...
#ifndef META_GLOBAL_REPOSITORY_ENTRY_H
#  define META_GLOBAL_REPOSITORY_ENTRY_H
...
#     define META_GLOBAL_REPOSITORY_ENTRY_DECL_SPEC DYNAMIC_EXPORT
#  else
#     define META_GLOBAL_REPOSITORY_ENTRY_DECL_SPEC DYNAMIC_IMPORT
#  endif

class META_GLOBAL_REPOSITORY_ENTRY_DECL_SPEC Meta_Global_Repository_Entry : public class_base
{
   public:
   typedef Meta_Global_Repository_Entry class_type;

   enum field_id
   {
      e_field_id_none = 0,
      e_field_id_File_Hash = 1,
      e_field_id_Local_Hash = 2,
      e_field_id_Local_Public_Key = 3,
      e_field_id_Master_Public_Key = 4
   };

   Meta_Global_Repository_Entry( );
   ~Meta_Global_Repository_Entry( );

   const std::string& File_Hash( ) const;
   void File_Hash( const std::string& File_Hash );

   const std::string& Local_Hash( ) const;
   void Local_Hash( const std::string& Local_Hash );

   const std::string& Local_Public_Key( ) const;
   void Local_Public_Key( const std::string& Local_Public_Key );

   const std::string& Master_Public_Key( ) const;
   void Master_Public_Key( const std::string& Master_Public_Key );
   ...
   static const char* static_class_id( ) { return "150100"; }
   static const char* static_class_name( ) { return "Global_Repository_Entry"; }
   static const char* static_plural_name( ) { return "Global_Repository_Entries"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_Global_Repository_Entry* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_Global_Repository_Entry( const Meta_Global_Repository_Entry& );
   Meta_Global_Repository_Entry& operator =( const Meta_Global_Repository_Entry& );
...
inline std::string to_string( const Meta_Global_Repository_Entry& c ) { return c.get_key( ); }

inline bool is_null( const Meta_Global_Repository_Entry& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_Global_Repository_Entry& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_Global_Repository_Entry& c, const char* p ) { return !( c.get_key( ) == p ); }
