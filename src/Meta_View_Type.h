...
#ifndef META_VIEW_TYPE_H
#  define META_VIEW_TYPE_H
...
class Meta_View_Type : public class_base
{
   ...
   public:
   typedef Meta_View_Type class_type;

   enum field_id
   {
      e_field_id_none = 0,
      e_field_id_Name = 1,
      e_field_id_View_Name = 2
   };
   ...
   void View_Name( const std::string& View_Name );

   Meta_View& child_View_Type( );
   const Meta_View& child_View_Type( ) const;
   ...
   static const char* static_class_id( ) { return "117100"; }
   static const char* static_class_name( ) { return "View_Type"; }
   static const char* static_plural_name( ) { return "View_Types"; }
   ...
   static void static_insert_external_alias( const std::string& module_and_class_id, Meta_View_Type* p_instance );
   static void static_remove_external_alias( const std::string& module_and_class_id );

   private:
   Meta_View_Type( const Meta_View_Type& );
   Meta_View_Type& operator =( const Meta_View_Type& );
...
inline std::string to_string( const Meta_View_Type& c ) { return c.get_key( ); }

inline bool is_null( const Meta_View_Type& c ) { return c.get_key( ).empty( ); }

inline bool check_equal( const Meta_View_Type& c, const char* p ) { return c.get_key( ) == p; }
inline bool check_not_equal( const Meta_View_Type& c, const char* p ) { return !( c.get_key( ) == p ); }
