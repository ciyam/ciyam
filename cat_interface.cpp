...
#ifndef _WIN32
const char* const c_kill_script = "cat_interface.kill.sh";
#else
const char* const c_kill_script = "cat_interface.kill.bat";
#endif
...
const char* const c_interface_file = "cat_interface.htms";
            ...
            else if( cmd == c_cmd_home )
            {
               ...
               if( !mod_info.home_list_id.empty( ) )
               {
                  output_list_form( extra_content,
                   ...
                   qlink, findinfo + listsrch, selected_records, !hashval.empty( ), false, has_any_changing_records );
               }
            }
            else if( cmd == c_cmd_view || cmd == c_cmd_pview )
            {
                  ...
                  if( !is_new_record && view.type != c_view_type_print && view.type != c_view_type_admin_print )
                  {
                     ...
                     for( size_t i = 0; i < child_lists.size( ); i++ )
                     {
                        ...
                        output_list_form( extra_content, child_lists[ i ],
                         ...
                         findinfo + listsrch, selected_records, !hashval.empty( ), is_owner, has_any_changing_records );
                     }
            ...
            else if( cmd == c_cmd_list || cmd == c_cmd_plist )
            {
               ...
               else
               {
                   ...
                   *p_session_info, specials, use_url_checksum, qlink, findinfo + listsrch,
                   selected_records, !hashval.empty( ), false, has_any_changing_records, &pdf_list_file_name );
