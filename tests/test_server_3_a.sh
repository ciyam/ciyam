...
echo "Check repeated usage of salted RPC password hash value fails."
echo -n $pass_hash >/tmp/ciyam/.rpc_password_hash
./ciyam_client -tls -quiet -no_stderr -rpc_unlock=@/tmp/ciyam/rpc_password.tmp "-exec=variable *devt*"
...
echo "Check repeated usage of salted RPC password hash value for prior ten seconds fails."
echo -n $pass_hash >/tmp/ciyam/.rpc_password_hash
./ciyam_client -tls -quiet -no_stderr -rpc_unlock=@/tmp/ciyam/rpc_password.tmp "-exec=variable *devt*"
...
# Simple API tests.
curl -s "localhost:13031/system"
curl -s "localhost:13031/ip-addr"
curl -s "localhost:13031/system?format=text"
curl -s "localhost:13031/system?format=json"
...
curl -s "localhost:13031/ip-addr"
curl -s -4 "localhost:13031/ip-addr?format=xxxx"
...
curl -s -X PUT "localhost:13031/cws/messages/0000002?access=$new_access&device=$new_device&options=name%3DUpdated&session=$new_session"
