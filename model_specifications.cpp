...
struct append_opts_for_iter_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
void append_opts_for_iter_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) < 3 )
      throw runtime_error( "unexpected number of args < 3 for 'append_opts_for_iter' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_field_name( args[ 1 ] );
   string arg_child_info( args[ 2 ] );
   string arg_cfield_info;
   ...
   for( size_t arg = 3; arg < args.size( ); arg++ )
   {
      ...
      if( arg_cfield_info.empty( ) )
         arg_cfield_info = next_arg;
      else if( next_arg.find( c_arg_sep_prefix ) == 0 )
         separator = next_arg.substr( strlen( c_arg_sep_prefix ) );
      else
         throw runtime_error( "unexpected extra argument '" + next_arg + "' for 'append_opts_for_iter' specification" );
   }
   ...
   string child_class_name( arg_child_info.substr( 0, pos ) );
   child_class_id = get_class_id_for_name( m, child_class_name );

   if( pos != string::npos )
   {
      order_field_id = get_field_id_for_name( m, child_class_name, arg_child_info.substr( pos + 1 ) );
      if( order_field_id.empty( ) )
         throw runtime_error( "unknown field '"
          + arg_child_info.substr( pos + 1 ) + "' for class '" + child_class_name + "'" );
   }
   ...
   if( !arg_cfield_info.empty( ) )
   {
      child_field_id = get_field_id_for_name( m, child_class_name, arg_cfield_info );
      if( child_field_id.empty( ) )
         throw runtime_error( "unknown field '" + arg_cfield_info + "' for class '" + child_class_name + "'" );
   }
...
void append_opts_for_iter_specification::read_data( sio_reader& reader )
{
...
void append_opts_for_iter_specification::write_data( sio_writer& writer ) const
{
...
void append_opts_for_iter_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   ...
   spec_data.data_pairs.push_back( make_pair( string( c_data_ofield ), order_field_name ) );
...
DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, append_opts_for_iter_specification, static_class_name );

struct check_child_match_with_original_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
void check_child_match_with_original_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) != 3 )
      throw runtime_error( "unexpected number of args != 3 for 'check_child_match_with_original' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_field_name( args[ 1 ] );
   string arg_pfield_info( args[ 2 ] );
   ...
   string::size_type pos = arg_pfield_info.find( ':' );
   if( pos == string::npos )
      throw runtime_error( "unexpected missing ':' in 'check_child_match_with_original' specification" );

   pfield_name = arg_pfield_info.substr( 0, pos );
   cfield_name = arg_pfield_info.substr( pos + 1 );
   ...
   cfield_id = get_field_id_for_name( m, pclass_name, cfield_name );
...
void check_child_match_with_original_specification::read_data( sio_reader& reader )
{
   class_id = reader.read_attribute( c_attribute_class_id );
...
void check_child_match_with_original_specification::write_data( sio_writer& writer ) const
{
   writer.write_attribute( c_attribute_class_id, class_id );
...
void check_child_match_with_original_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   string class_name = get_class_name_for_id( m, class_id );
   spec_data.data_pairs.push_back( make_pair( c_data_class, class_name ) );
...
string check_child_match_with_original_specification::static_class_name( ) { return "check_child_match_with_original"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, check_child_match_with_original_specification, static_class_name );
...
struct check_null_or_gteq_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
void check_null_or_gteq_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) != 3 )
      throw runtime_error( "unexpected number of args != 3 for 'check_null_or_gteq' specification" );
   ...
   field2_id = get_field_id_for_name( m, arg_class_name, arg_field2_name );
   if( field2_id.empty( ) )
      throw runtime_error( "unknown field '" + arg_field2_name + "' for class '" + arg_class_name + "'" );
...
string check_null_or_gteq_specification::static_class_name( ) { return "check_null_or_gteq"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, check_null_or_gteq_specification, static_class_name );

struct check_parent_fk_match_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
   ...
   string parent_check_field_id;
};

void check_parent_fk_match_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) < 3 )
      throw runtime_error( "unexpected number of args < 3 for 'check_parent_fk_match' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_field_name( args[ 1 ] );
   string arg_parent_field_name( args[ 2 ] );

   string arg_parent_check_field_name( arg_field_name );

   if( args.size( ) > 3 )
   {
      if( args.size( ) == 4 )
         arg_parent_check_field_name = args[ 3 ];
      else
         throw runtime_error( "unexpected extra args for 'check_parent_fk_match' specification" );
   }
   ...
   parent_field_id = get_field_id_for_name( m, arg_class_name,
    arg_parent_field_name, &parent_field_type, true, false, true );
   ...
   if( parent_class_name.empty( ) )
      throw runtime_error( "unexpected non-parent field '"
       + arg_parent_field_name + "' was provided to 'check_parent_fk_match' specification" );

   bool is_external_parent;
   parent_class_id = get_class_id_for_name( m, parent_class_name, &is_external_parent );

   parent_check_field_id = get_field_id_for_name( m,
    parent_class_name, arg_parent_check_field_name, 0, true, false, true );

   if( parent_check_field_id.empty( ) )
      throw runtime_error( "unknown field '"
       + arg_parent_check_field_name + "' for class '" + parent_class_name + "'" );
   ...
   if( !is_external_parent )
   {
      ...
      details.push_back( specification_detail( parent_check_field_id, "cfield", e_model_element_type_field ) );
   }
}

void check_parent_fk_match_specification::read_data( sio_reader& reader )
{
   class_id = reader.read_attribute( c_attribute_class_id );
   field_id = reader.read_attribute( c_attribute_field_id );
   parent_class_id = reader.read_attribute( c_attribute_pclass_id );
   parent_field_id = reader.read_attribute( c_attribute_pfield_id );
   parent_check_field_id = reader.read_attribute( c_attribute_cfield_id );
}

void check_parent_fk_match_specification::write_data( sio_writer& writer ) const
{
   writer.write_attribute( c_attribute_class_id, class_id );
   writer.write_attribute( c_attribute_field_id, field_id );
   writer.write_attribute( c_attribute_pclass_id, parent_class_id );
   writer.write_attribute( c_attribute_pfield_id, parent_field_id );
   writer.write_attribute( c_attribute_cfield_id, parent_check_field_id );
}

void check_parent_fk_match_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   string class_name = get_class_name_for_id( m, class_id );
   spec_data.data_pairs.push_back( make_pair( c_data_class, class_name ) );
   ...
   string parent_field_name = get_field_name_for_id( m, class_name, parent_field_id, 0, true );
   spec_data.data_pairs.push_back( make_pair( c_data_pfield, parent_field_name ) );

   string parent_check_field_name = get_field_name_for_id( m, parent_class_name, parent_check_field_id, 0, true );
   spec_data.data_pairs.push_back( make_pair( c_data_cfield, parent_check_field_name ) );
}

string check_parent_fk_match_specification::static_class_name( ) { return "check_parent_fk_match"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, check_parent_fk_match_specification, static_class_name );
...
struct child_status_cascade_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
   ...
   string status_value;
   string order_field_id;
   string special_field_id;
   string special_class_id;
   string special_cfield_id;
   string special_value;
   string neq_status_field_id;
   string neq_status_value;

   string special_extra_class_id;
   string special_extra_source_id;
   vector< string > special_extra_fields;
};

void child_status_cascade_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) < 4 )
      throw runtime_error( "unexpected number of args < 4 for 'child_status_cascade' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_child_name( args[ 1 ] );
   string arg_child_field_name( args[ 2 ] );
   string arg_status_value( args[ 3 ] );

   string arg_order_info, arg_special_info, arg_neq_status_info, arg_special_extra_info;
   ...
   for( size_t arg = 4; arg < args.size( ); arg++ )
   {
      ...
      else if( next_arg.find( c_arg_special_prefix ) == 0 )
         arg_special_info = next_arg.substr( strlen( c_arg_special_prefix ) );
      else if( next_arg.find( c_arg_neq_status_prefix ) == 0 )
         arg_neq_status_info = next_arg.substr( strlen( c_arg_neq_status_prefix ) );
      else if( next_arg.find( c_arg_special_extra_prefix ) == 0 )
         arg_special_extra_info = next_arg.substr( strlen( c_arg_special_extra_prefix ) );
      else
         throw runtime_error( "unexpected extra argument '" + next_arg + "' for 'child_status_cascade' specification" );
   }

   class_id = get_class_id_for_name( m, arg_class_name );

   string relationship_name( decorated_class_name( arg_class_name ) );
   ...
   status_value = arg_status_value;

   string special_class_name, special_extra_class_name;
   ...
   if( !arg_special_info.empty( ) )
   {
      string::size_type pos = arg_special_info.rfind( ',' );
      if( pos == string::npos )
         throw runtime_error( "unexpected format for special info '" + arg_special_info + "'" );

      string special_fields( arg_special_info.substr( 0, pos ) );
      special_value = arg_special_info.substr( pos + 1 );

      pos = special_fields.find( ',' );
      if( pos == string::npos )
         throw runtime_error( "unexpected format for special fields '" + special_fields + "'" );

      special_field_id = get_field_id_for_name( m, arg_class_name, special_fields.substr( 0, pos ), 0, true );
      if( special_field_id.empty( ) )
         throw runtime_error( "unknown special field '" + special_fields.substr( 0, pos ) + "' for class '" + arg_class_name + "'" );

      string special_type;
      special_cfield_id = get_field_id_for_name( m, rel_data.child_class_name, special_fields.substr( pos + 1 ), &special_type, true );
      if( special_cfield_id.empty( ) )
         throw runtime_error( "unknown special child field '"
          + special_fields.substr( pos + 1 ) + "' for class '" + rel_data.child_class_name + "'" );

      special_class_name = get_class_name_from_field_type( m,
       rel_data.child_class_name, special_fields.substr( pos + 1 ), special_type );
      special_class_id = get_class_id_for_name( m, special_class_name );

      if( special_class_id.empty( ) )
         throw runtime_error( "field '" + special_fields.substr( pos + 1 ) + "' is not a foreign key in '" + rel_data.child_class_name + "'" );
   }

   if( !arg_neq_status_info.empty( ) )
   {
      string::size_type pos = arg_neq_status_info.find( ',' );
      if( pos == string::npos )
         throw runtime_error( "unexpected format for neq_status info '" + arg_neq_status_info + "'" );

      neq_status_field_id = get_field_id_for_name( m, arg_class_name, arg_neq_status_info.substr( 0, pos ), 0, true );
      if( neq_status_field_id.empty( ) )
         throw runtime_error( "unknown neq_status field '"
          + arg_neq_status_info.substr( 0, pos ) + "' for class '" + arg_class_name + "'" );

      neq_status_value = arg_neq_status_info.substr( pos + 1 );
   }

   if( !arg_special_extra_info.empty( ) )
   {
      string::size_type pos = arg_special_extra_info.find( ':' );
      if( pos == string::npos )
         throw runtime_error( "unexpected format for special_extra info '" + arg_special_extra_info + "'" );

      string special_extra_source_name( arg_special_extra_info.substr( 0, pos ) );
      arg_special_extra_info.erase( 0, pos + 1 );

      string special_extra_type;
      special_extra_source_id = get_field_id_for_name( m, rel_data.child_class_name, special_extra_source_name, &special_extra_type, true );
      if( special_extra_source_id.empty( ) )
         throw runtime_error( "unknown special extra source '" + special_extra_source_name + "' for class '" + rel_data.child_class_name + "'" );

      special_extra_class_name = get_class_name_from_field_type( m,
       rel_data.child_class_name, special_extra_source_name, special_extra_type );
      special_extra_class_id = get_class_id_for_name( m, special_extra_class_name );

      if( special_extra_class_id.empty( ) )
         throw runtime_error( "field '" + special_extra_source_name + "' is not a foreign key in '" + rel_data.child_class_name + "'" );

      while( true )
      {
         pos = arg_special_extra_info.find( ',' );

         string next_field( arg_special_extra_info.substr( 0, pos ) );

         string next_special_field_id = get_field_id_for_name( m, special_class_name, next_field, 0, true );
         string next_special_extra_field_id = get_field_id_for_name( m, special_extra_class_name, next_field, 0, true );

         if( next_special_field_id.empty( ) )
            throw runtime_error( "unknown field '" + next_field + "' for class '" + special_class_name + "'" );

         if( next_special_extra_field_id.empty( ) )
            throw runtime_error( "unknown field '" + next_field + "' for class '" + special_extra_class_name + "'" );

         special_extra_fields.push_back( next_special_field_id + ':' + next_special_extra_field_id );
   ...
   if( !special_class_id.empty( ) )
      details.push_back( specification_detail( special_class_id, "special_class", e_model_element_type_class ) );

   if( !special_extra_class_id.empty( ) )
      details.push_back( specification_detail( special_extra_class_id, "special_extra_class", e_model_element_type_class ) );
   ...
   if( !special_field_id.empty( ) )
      details.push_back( specification_detail( special_field_id, "special_field", e_model_element_type_field ) );

   if( !special_cfield_id.empty( ) )
      details.push_back( specification_detail( special_cfield_id, "special_cfield", e_model_element_type_field ) );

   if( !neq_status_field_id.empty( ) )
      details.push_back( specification_detail( neq_status_field_id, "neq_status_field", e_model_element_type_field ) );

   if( !special_extra_source_id.empty( ) )
      details.push_back( specification_detail( special_extra_source_id, "special_extra_source", e_model_element_type_field ) );

   details.push_back( specification_detail( relationship_id, "relationship", e_model_element_type_relationship ) );
}

void child_status_cascade_specification::read_data( sio_reader& reader )
{
   class_id = reader.read_attribute( c_attribute_class_id );
   relationship_id = reader.read_attribute( c_attribute_relationship_id );
   child_class_id = reader.read_attribute( c_attribute_child_class_id );
   child_field_id = reader.read_attribute( c_attribute_child_field_id );
   status_value = reader.read_attribute( c_attribute_status_value );

   order_field_id = reader.read_opt_attribute( c_attribute_order_field_id );
   special_field_id = reader.read_opt_attribute( c_attribute_special_field_id );
   special_class_id = reader.read_opt_attribute( c_attribute_special_class_id );
   special_cfield_id = reader.read_opt_attribute( c_attribute_special_cfield_id );
   special_value = reader.read_opt_attribute( c_attribute_special_value );
   neq_status_field_id = reader.read_opt_attribute( c_attribute_neq_status_field_id );
   neq_status_value = reader.read_opt_attribute( c_attribute_neq_status_value );
   special_extra_class_id = reader.read_opt_attribute( c_attribute_special_extra_class_id );
   special_extra_source_id = reader.read_opt_attribute( c_attribute_special_extra_source_id );

   string next;
   while( reader.has_read_attribute( c_attribute_special_extra_field_info, next ) )
      special_extra_fields.push_back( next );
}

void child_status_cascade_specification::write_data( sio_writer& writer ) const
{
   writer.write_attribute( c_attribute_class_id, class_id );
   writer.write_attribute( c_attribute_relationship_id, relationship_id );
   writer.write_attribute( c_attribute_child_class_id, child_class_id );
   writer.write_attribute( c_attribute_child_field_id, child_field_id );
   writer.write_attribute( c_attribute_status_value, status_value );

   writer.write_opt_attribute( c_attribute_order_field_id, order_field_id );
   writer.write_opt_attribute( c_attribute_special_field_id, special_field_id );
   writer.write_opt_attribute( c_attribute_special_class_id, special_class_id );
   writer.write_opt_attribute( c_attribute_special_cfield_id, special_cfield_id );
   writer.write_opt_attribute( c_attribute_special_value, special_value );
   writer.write_opt_attribute( c_attribute_neq_status_field_id, neq_status_field_id );
   writer.write_opt_attribute( c_attribute_neq_status_value, neq_status_value );
   writer.write_opt_attribute( c_attribute_special_extra_class_id, special_extra_class_id );
   writer.write_opt_attribute( c_attribute_special_extra_source_id, special_extra_source_id );

   for( size_t i = 0; i < special_extra_fields.size( ); i++ )
      writer.write_attribute( c_attribute_special_extra_field_info, special_extra_fields[ i ] );
}

void child_status_cascade_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   string class_name = get_class_name_for_id( m, class_id );
   spec_data.data_pairs.push_back( make_pair( c_data_class, class_name ) );
   ...
   string child_field_name = get_field_name_for_id( m, child_class_name, child_field_id );
   ...
   if( !special_field_id.empty( ) )
      special_field_name = get_field_name_for_id( m, class_name, special_field_id );
   spec_data.data_pairs.push_back( make_pair( c_data_special_field, special_field_name ) );

   string special_cfield_name;
   if( !special_cfield_id.empty( ) )
      special_cfield_name = get_field_name_for_id( m, child_class_name, special_cfield_id );
   spec_data.data_pairs.push_back( make_pair( c_data_special_cfield, special_cfield_name ) );

   spec_data.data_pairs.push_back( make_pair( c_data_special_value, special_value ) );

   string neq_status_field_name;
   if( !neq_status_field_id.empty( ) )
      neq_status_field_name = get_field_name_for_id( m, class_name, neq_status_field_id );
   spec_data.data_pairs.push_back( make_pair( c_data_neq_status_field, neq_status_field_name ) );

   spec_data.data_pairs.push_back( make_pair( c_data_neq_status_value, neq_status_value ) );

   string special_extra_source_name;
   if( !special_extra_source_id.empty( ) )
      special_extra_source_name = get_field_name_for_id( m, child_class_name, special_extra_source_id );

   spec_data.data_pairs.push_back( make_pair( c_data_special_extra_source, special_extra_source_name ) );

   string special_class_name;
   if( !special_class_id.empty( ) )
      special_class_name = get_class_name_for_id( m, special_class_id );

   string special_extra_class_name;
   if( !special_extra_class_id.empty( ) )
      special_extra_class_name = get_class_name_for_id( m, special_extra_class_id );

   string special_extra_field_info;
   for( size_t i = 0; i < special_extra_fields.size( ); i++ )
   {
      if( !special_extra_field_info.empty( ) )
         special_extra_field_info += ' ';

      size_t pos = special_extra_fields[ i ].find( ':' );

      special_extra_field_info += get_field_name_for_id( m, special_class_name, special_extra_fields[ i ].substr( 0, pos ) );
      special_extra_field_info += ',';
      special_extra_field_info += get_field_name_for_id( m, special_extra_class_name, special_extra_fields[ i ].substr( pos + 1 ) );
   }

   spec_data.data_pairs.push_back( make_pair( c_data_special_extra_field_info, special_extra_field_info ) );
}

string child_status_cascade_specification::static_class_name( ) { return "child_status_cascade"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, child_status_cascade_specification, static_class_name );
...
struct field_cond_concatenate_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
void field_cond_concatenate_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) != 5 )
      throw runtime_error( "unexpected number of args != 5 for 'field_cond_concatenate' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_src_field_name( args[ 1 ] );
   string arg_src_field_value( args[ 2 ] );
   string arg_dest_field_name( args[ 3 ] );
   string arg_dest_field_value( args[ 4 ] );

   string class_id = get_class_id_for_name( m, arg_class_name );

   string src_field_id, src_field_type;
   src_field_id = get_field_id_for_name( m, arg_class_name, arg_src_field_name, &src_field_type );

   if( src_field_id.empty( ) )
      throw runtime_error( "unknown field name '" + arg_src_field_name + "' for class '" + arg_class_name + "'" );

   bool is_non_string( is_non_string_type( src_field_type ) );

   if( is_non_string )
      test_value = arg_src_field_value;
   else
      test_value = '"' + arg_src_field_value + '"';

   string dest_field_id = get_field_id_for_name( m, arg_class_name, arg_dest_field_name );

   if( dest_field_id.empty( ) )
      throw runtime_error( "unknown field name '" + arg_dest_field_name + "' for class '" + arg_class_name + "'" );

   dest_value = arg_dest_field_value;
...
void field_cond_concatenate_specification::read_data( sio_reader& reader )
{
   test_value = reader.read_attribute( c_attribute_test_value );
   dest_value = reader.read_attribute( c_attribute_dest_value );
}

void field_cond_concatenate_specification::write_data( sio_writer& writer ) const
{
   writer.write_attribute( c_attribute_test_value, test_value );
   writer.write_attribute( c_attribute_dest_value, dest_value );
}

void field_cond_concatenate_specification::add_specification_data( model& /*m*/, specification_data& spec_data ) const
{
   spec_data.data_pairs.push_back( make_pair( c_data_tvalue, test_value ) );
   spec_data.data_pairs.push_back( make_pair( c_data_dvalue, dest_value ) );
}

string field_cond_concatenate_specification::static_class_name( ) { return "field_cond_concatenate"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, field_cond_concatenate_specification, static_class_name );
...
struct field_from_addition_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
void field_from_addition_specification::add( model& m, const vector< string >& args, vector< specification_detail >& details )
{
   if( args.size( ) < 4 )
      throw runtime_error( "unexpected number of args < 4 for 'field_from_addition' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_dest_field_name( args[ 1 ] );
   string arg_src_field_name( args[ 2 ] );
   string arg_value( args[ 3 ] );
   ...
   src_field_id = get_field_id_for_name( m, arg_class_name, arg_src_field_name );

   if( src_field_id.empty( ) )
      throw runtime_error( "unknown field name '" + arg_src_field_name + "' for class '" + arg_class_name + "'" );

   data_value = arg_value;
   ...
   for( size_t arg = 4; arg < args.size( ); arg++ )
   {
      ...
      if( next_arg == c_arg_not_default )
         not_default = true;
      else if( next_arg.find( c_arg_type_prefix ) == 0 )
         data_type = next_arg.substr( strlen( c_arg_type_prefix ) );
      else
         throw runtime_error( "unexpected extra argument '" + next_arg + "' for 'field_from_addition' specification" );
   }
...
void field_from_addition_specification::read_data( sio_reader& reader )
{
   class_id = reader.read_attribute( c_attribute_class_id );
   data_type = reader.read_attribute( c_attribute_data_type );
   data_value = reader.read_attribute( c_attribute_data_value );
...
void field_from_addition_specification::write_data( sio_writer& writer ) const
{
   writer.write_attribute( c_attribute_class_id, class_id );
   writer.write_attribute( c_attribute_data_type, data_type );
   writer.write_attribute( c_attribute_data_value, data_value );
...
void field_from_addition_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   string class_name = get_class_name_for_id( m, class_id );
   spec_data.data_pairs.push_back( make_pair( c_data_class, class_name ) );

   spec_data.data_pairs.push_back( make_pair( c_data_dtype, data_type ) );
   spec_data.data_pairs.push_back( make_pair( c_data_value, data_value ) );

   string src_field_name = get_field_name_for_id( m, class_name, src_field_id );
   spec_data.data_pairs.push_back( make_pair( c_data_sfield, src_field_name ) );

   string dest_field_name = get_field_name_for_id( m, class_name, dest_field_id );
   spec_data.data_pairs.push_back( make_pair( c_data_dfield, dest_field_name ) );

   spec_data.data_pairs.push_back( make_pair( c_data_not_dflt, not_default ? c_true : "" ) );
}

string field_from_addition_specification::static_class_name( ) { return "field_from_addition"; }

DEFINE_CLASS_FACTORY_INSTANTIATOR( string, specification, field_from_addition_specification, static_class_name );
...
struct prior_version_specification : specification
{
   void add( model& m, const vector< string >& args, vector< specification_detail >& details );
...
{
   if( args.size( ) < 4 )
      throw runtime_error( "unexpected number of args < 4 for 'prior_version' specification" );

   string arg_class_name( args[ 0 ] );
   string arg_prior_version_class_name( args[ 1 ] );
   ...
   for( size_t arg = 4; arg < args.size( ); arg++ )
   {
      ...
      if( next_arg.find( c_arg_dtm_prefix ) == 0 )
         ...
         throw runtime_error( "unexpected extra argument '" + next_arg + "' for prior_version specification" );
   }

   class_id = get_class_id_for_name( m, arg_class_name );
   prior_version_class_id = get_class_id_for_name( m, arg_prior_version_class_name );
   ...
   set< string > alias_field_ids;

   vector< string > fields;
   split( arg_field_names, fields );

   vector< string > field_ids;
   for( size_t i = 0; i < fields.size( ); i++ )
   {
      string::size_type pos = fields[ i ].find( ':' );
      string next_field_name( fields[ i ].substr( 0, pos ) );
      string next_field_type;

      string next_field_id( get_field_id_for_name( m, arg_class_name, next_field_name, &next_field_type, pos != string::npos ) );

      if( next_field_id.empty( ) )
         throw runtime_error( "unknown field '" + next_field_name + "' for class '" + arg_class_name + "'" );

      field_ids.push_back( next_field_id );

      if( pos != string::npos )
      {
         string fk_class_name( get_class_name_from_field_type( m, arg_class_name, next_field_name, next_field_type ) );

         bool is_external_alias;
         get_class_id_for_name( m, fk_class_name, &is_external_alias );

         string fk_field_id( get_field_id_for_name( m, fk_class_name, fields[ i ].substr( pos + 1 ), 0, false, false, true ) );

         if( fk_field_id.empty( ) )
            throw runtime_error( "unknown field '" + fields[ i ].substr( pos + 1 ) + "' for fk class '" + fk_class_name + "'" );

         field_ids.push_back( fk_field_id );

         if( is_external_alias )
            alias_field_ids.insert( fk_field_id );

         next_field_id += ':';
         next_field_id += fk_field_id;
      }
   ...
   if( !arg_test_info.empty( ) )
   {
      ...
      test_field_id = get_field_id_for_name( m, arg_class_name, arg_test_info.substr( 0, pos ), 0, true );
   ...
   for( size_t i = 0; i < field_ids.size( ); i++ )
   {
      if( alias_field_ids.count( field_ids[ i ] ) )
         continue;
      details.push_back( specification_detail( field_ids[ i ], "field", e_model_element_type_field ) );
   }
...
{
   class_id = reader.read_attribute( c_attribute_class_id );
   prior_version_class_id = reader.read_attribute( c_attribute_prior_version_class_id );
   version_field_id = reader.read_attribute( c_attribute_prior_version_ver_field_id );
...
{
   writer.write_attribute( c_attribute_class_id, class_id );
   writer.write_attribute( c_attribute_prior_version_class_id, prior_version_class_id );
   writer.write_attribute( c_attribute_prior_version_ver_field_id, version_field_id );
...
void prior_version_specification::add_specification_data( model& m, specification_data& spec_data ) const
{
   ...
   {
      string::size_type pos = field_info[ i ].find( ':' );
      string next_field_id( field_info[ i ].substr( 0, pos ) );

      string next_field_type;
      string next_field_name( get_field_name_for_id( m, class_name, next_field_id, &next_field_type ) );

      if( i > 0 )
         field_list += ' ';

      field_list += next_field_name;

      string next_pfield_name;
      if( pos != string::npos )
      {
         string fk_class_name( get_class_name_from_field_type( m, class_name, next_field_name, next_field_type ) );

         if( fk_class_name.empty( ) )
            throw runtime_error( "field '" + next_field_name + "' is not a foreign key" );

         next_pfield_name = get_field_name_for_id( m, fk_class_name, field_info[ i ].substr( pos + 1 ), 0, true );
      }

      field_list += ',' + next_pfield_name;
   }
